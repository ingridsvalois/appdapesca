...
export default function AdminPedidos() {
  ...
  const [segment, setSegment] = useState<
    "unpaid" | "processing" | "shipped" | "delivered" | "cancelled"
  >("processing");
  ...
  function loadCounts() {
    ...
    Promise.all(
      segments.map((seg) =>
        apiGet<{ total: number }>(
          `/api/admin/orders?segment=${seg}&page=1&limit=1`
        )
  ...
  function getStatusBadge(order: OrderItem) {
        ...
        {[
          ...
          return (
            <button
              key={tab.key}
              type="button"
              onClick={() => setSegment(tab.key as typeof segment)}
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-sm font-semibold text-[#333333]">
                    Pedido
                  </th>
                  <th className="px-4 py-3 text-sm font-semibold text-[#333333]">
                    Cliente
                  ...
                  return (
                    <tr key={order.id}>
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs inline-flex items-center px-2.5 py-0.5 rounded-full font-medium ${badge.className}`}
                        >
                          {badge.label}
                        </span>
                      </td>
